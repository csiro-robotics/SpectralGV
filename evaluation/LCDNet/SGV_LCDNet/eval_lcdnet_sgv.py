...
# The folder containing this script should be copied into LCDNet/evaluation/ before executing.
...
class MetLocEvaluator(Evaluator):
    ...
    def evaluate(self, model, *args, **kwargs):
        ...
        save_path = os.path.dirname(__file__) + '/pickles/lcdnet_' + self.dataset_type + '/'

        query_embeddings, local_query_embeddings = self.compute_embeddings(self.eval_set.query_set, model, self.only_global)
        ...
        map_embeddings, local_map_embeddings = self.compute_embeddings(self.eval_set.map_set, model, self.only_global)
        ...
        for query_ndx in tqdm.tqdm(query_indexes):
            ...
            query_embedding = query_embeddings[query_ndx]
            embed_dist = np.linalg.norm(map_embeddings - query_embedding, axis=1)
            # embed_dist = torch.cdist(torch.tensor(query_embedding).unsqueeze(0).unsqueeze(0), torch.tensor(map_embeddings).unsqueeze(0)).squeeze().numpy()
            nn_ndx = np.argsort(embed_dist)[:self.k]
    ...
    def compute_embeddings(self, eval_subset: List[EvaluationTuple], model, global_only=False):
        self.model2eval((model,))
        ...
        for ndx, e in tqdm.tqdm(enumerate(eval_subset)):
            ...
            if global_only:
                global_embedding = self.compute_embedding(xyzi, model, global_only=global_only)
                ...
                global_embedding, keypoints, key_embeddings = self.compute_embedding(xyzi, model)
                # keypoints, key_embeddings = 1, 1
                if global_embeddings is None:
                    global_embeddings = np.zeros((len(eval_subset), global_embedding.shape[1]), dtype=global_embedding.dtype)
    ...
    def compute_embedding(self, pc, model, global_only=False):
        ...
        # batch_dict = model(model_in, metric_head=False, compute_rotation=False, compute_transl=False)
            ...
            feats = batch_dict['point_features'].squeeze(-1).squeeze()

            coords = coords.squeeze()[:, 1:].cpu().detach()#.numpy()
            feats = feats.permute(1, 0).cpu().detach()#.numpy()
            ...
            return global_embedding, coords, feats
    ...
    def print_results(self, global_metrics, metrics):
        ...
        for r in recall:
            ...
            print("")
            print('MRR: ', global_metrics['MRR'][r])
        ...
        for r_rr in recall_rr:
            ...
            for x in recall_rr[r_rr]:
                print("{:0.3f}, ".format(x), end='')
            print("")
            print('MRR: ', global_metrics['MRR_rr'][r_rr])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_type', type=str, required=False, default='kitti360', choices=['mulran', 'southbay', 'kitti', 'ugv', 'kitti360', 'bushwalk'])
    ...
    parser.add_argument('--weights', type=str, default='lcdnet.tar', help='Trained global model weights')
    ...
    weights_path = os.path.dirname(__file__) + '/../../checkpoints/' + args.weights #'LCDNet-kitti360.tar'
    ...
    from get_models_cfg import get_model
    ...
    evaluator = MetLocEvaluator(args.dataset_root, args.dataset_type, args.eval_set, device, radius=args.radius, k=20,
                                   n_samples=args.n_samples, icp_refine=args.icp_refine)
