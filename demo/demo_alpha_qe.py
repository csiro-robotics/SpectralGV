...
# Outputs results for place recognition both with and without re-ranking using alpha-QE.
...
class MetLocEvaluatorDemo(EvaluatorDemo):
    ...
    def evaluate(self):
        ...
        for query_ndx in tqdm.tqdm(query_indexes):
            ...
            tick = time()
            topk_gds = map_embeddings[nn_ndx[:topk]]
            topk_gd_dists = embed_dist[nn_ndx[:topk]]
            new_q = np.vstack([topk_gds[j] * topk_gd_dists[j]**alpha for j in range(len(topk_gds))])
            new_q = np.vstack([new_q, query_embedding])
            alpha_avg_descriptor = np.average(new_q, axis=0)
            rr_embed_dist = np.linalg.norm(map_embeddings - alpha_avg_descriptor, axis=1)
